#!/usr/bin/env bash
# install-codex.sh — wire video-to-notebook into the OpenAI Codex CLI.
#
...
#   - When run inside a video-to-notebook study project: symlinks the repo's
#     AGENTS.md into the cwd so Codex picks it up locally.
#   - When run with --global: writes a stanza into ~/.codex/AGENTS.md that
#     points Codex to the video-to-notebook CLI conventions globally.
#
# Usage:
...
usage() {
  cat <<EOF
install-codex.sh — wire video-to-notebook into OpenAI Codex CLI
...
Prereqs:
  ...
  - video-to-notebook CLI installed (\`pip install video-to-notebook\`)

After install, start Codex from your study-project directory:
  cd my-study-site
  codex          # Codex reads AGENTS.md, knows how to drive video-to-notebook
EOF
}
...
case "${1:-}" in
  ...
  --global)
    ...
    if grep -q "video-to-notebook" "$GLOBAL_AGENTS" 2>/dev/null; then
      echo ">>> ~/.codex/AGENTS.md already mentions video-to-notebook — skipping."
      exit 0
    fi
...
\`video-to-notebook\` CLI. Each LLM stage supports \`--print-prompts\` / \`--apply-results\`
...
EOF
    echo "appended video-to-notebook pointer to $GLOBAL_AGENTS"
    echo ""
    echo "Now \`codex\` will know about video-to-notebook from any directory."
    ;;
  ""|--project)
    ...
    echo "the video-to-notebook pipeline. Tell it something like:"
    echo ""
