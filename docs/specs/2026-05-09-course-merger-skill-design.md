...
- `pip install course-merger` — for CLI users.
...
|---------|------|----------|------|
| `init [--ontology PATH] [--force]` | optional seed concepts YAML | scaffold `.course-merger/{db.sqlite, config.toml, site/}`; if ontology, insert concepts with `ontology_source='seed'` | errors if already initialized unless `--force` |
...
| `review [--type T]` | items in `.course-merger/review/` | opens markdown queue files for human disambiguation; writes adjudication back to DB | clears the queue once committed |
...
| cluster | LLM ambiguous (merge-vs-new uncertain) | write `.course-merger/review/cluster_<hash>.md` with options; CLI prints "N items pending review — run `course-merger review`" |
...
| Distribution | `pip install course-merger` + Claude Code skill plugin | one CLI, two entry points |
| Repo name | `course-merger` (working title; bikeshedding allowed) | descriptive |
