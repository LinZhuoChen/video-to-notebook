...
**Repo:** `/Users/chenlinzhuo/code/course-merger/` (at tag `plan-2-done`, commit `fcb3666`).
...
"template-site" = "course_merger/_template_site"
```

This packages `template-site/` under `course_merger/_template_site/` inside the installed wheel, so the runtime can find it via importlib.resources.
...
**Files:**
- Create: `src/course_merger/build/__init__.py` (empty)
- Create: `src/course_merger/build/queries.py`
...
```bash
cd /Users/chenlinzhuo/code/course-merger && .venv/bin/pytest tests/unit/test_build_queries.py -v
```

- [ ] **Step 3: Write `src/course_merger/build/__init__.py`** (empty file)

- [ ] **Step 4: Write `src/course_merger/build/queries.py`**
...
```bash
git add src/course_merger/build/__init__.py src/course_merger/build/queries.py tests/unit/test_build_queries.py
...
**Files:**
- Create: `src/course_merger/build/writers.py`
...
- [ ] **Step 3: Implement `src/course_merger/build/writers.py`**
...
```bash
git add src/course_merger/build/writers.py tests/unit/test_build_writers.py
...
**Files:**
- Create: `src/course_merger/build/template_copy.py`
...
- [ ] **Step 3: Write `src/course_merger/build/template_copy.py`**
...
def _bundled_template_root() -> Path:
    ...
    `template-site/`; after `pip install`, to `course_merger/_template_site/`.
    """
...
```bash
git add src/course_merger/build/template_copy.py tests/unit/test_template_copy.py
...
**Files:**
- Create: `src/course_merger/build/runner.py`
...
- [ ] **Step 3: Write `src/course_merger/build/runner.py`**
...
```bash
git add src/course_merger/build/runner.py tests/unit/test_build_runner.py
...
```bash
cd /Users/chenlinzhuo/code/course-merger/template-site && npm run build 2>&1 | tail -10
```
...
```bash
cd /Users/chenlinzhuo/code/course-merger/template-site && npm run build 2>&1 | tail -15
```
...
```bash
cd /Users/chenlinzhuo/code/course-merger/template-site && npm run build 2>&1 | tail -5
```
...
**Files:**
- Modify: `src/course_merger/cli.py` — add `build` and `serve` commands
...
- [ ] **Step 3: Modify `src/course_merger/cli.py`**
...
```bash
git add src/course_merger/cli.py tests/integration/test_build_smoke.py
...
```bash
cd /tmp/cm-plan2
uv run --project /Users/chenlinzhuo/code/course-merger course-merger build
```
...
```bash
uv run --project /Users/chenlinzhuo/code/course-merger course-merger serve
...
uv run --project /Users/chenlinzhuo/code/course-merger course-merger build --incremental --no-npm
```
...
- GitHub Pages deployment workflow: a new `.github/workflows/pages.yml` that runs `course-merger build` in the demo subproject and uploads `site/dist/`.
- `skills/course-merger/SKILL.md`: a markdown skill plugin file that maps natural-language requests ("crawl this course", "build the site") to the CLI commands.
