...
**Goal:** Pivot course-merger from "concept index" to "textbook generator". Given N crawled+tagged+clustered courses, produce a beginner-friendly multi-chapter HTML textbook with pedagogical sequencing, inline SVG diagrams, CSS animations, embedded source-video clips, per-chapter takeaways.
...
**Repo:** `/Users/chenlinzhuo/code/course-merger/` (at tag `v1.1.0`).
...
- **T2**: `curriculum/prompt_io.py` with `collect_curriculum_prompts` + `apply_curriculum_results`. CLI: `course-merger curriculum --print-prompts/--apply-results`.
- **T3**: `synthesize/prompt_io.py` with `collect_synthesize_prompts(chapter_order_idx)` + `apply_synthesize_results`. CLI: `course-merger synthesize --chapter N --print-prompts/--apply-results <path>`.
- **T4**: `build/textbook_writer.py` copies `.course-merger/textbook/N.html` → `site/src/content/textbook/N.html`, writes `curriculum.json` manifest. Wired into `run_build`.
